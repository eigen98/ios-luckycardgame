...
## [1-2] 럭키 카드 클래스 구현하기(4시간) - 화요일 2시간
...
## [1-3] 카드 나눠주기(9시간) - 수요일 6시간 & 목요일 3시간
...
# 2주차 계획.    
## [1-3]과제 리팩토링(8시간) - 월요일(7월 10일)
1. **뷰 컴포넌트로 관리하기(2시간)**
    - CardContainerView 뷰 클래스로 만들어 분리 (1시간) ✅
    - BottomContainerView 뷰 클래스로 만들어 분리 (1시간) ✅
    
    
2. **게임 비즈니스 로직 구현(4시간)**
    - GameService 구현 : 게임 관리자(GameManager)를 래핑하여 게임 플레이의 로직을 관리하는 객체 (1시간 30분) ✅
    - GameManager 구현 : 게임의 핵심 로직과 상태를 관리하는 구조체 (2시간) ✅
    - GameMode 구현 : 게임의 모드를 정의하는 열거형  (30분) ✅
3. **Integration (4시간)**
    - 레이아웃 및 프로퍼티 설정 함수 구현(2시간) ✅
    - 지난 시간에 나온 PR을 반영한 리팩토링(2시간) ✅
### 결과물
https://github.com/eigen98/eigen98/assets/68258365/4e101773-1b59-4458-981e-a2310147e087    


## [2-1]게임 로직 구현하기(7시간) - 화요일(7월 11일)
[1-4]에 이어서 동일한 과제 진행.
1. **XCTest 학습 (2시간)**
    - XCTest 영상 자료 시청 (40분) ✅
    - XCTest 영상 자료 정리 (1시간 20분) ✅

2. ** 테스트 기능 정리 및 미리 생각해보기 (1시간)**
    - LuckyGame 테스트 기능 생각해보기 및 결함 분석(30분)
    - GameManager 테스트 기능 생각해보기 및 결함 분석(30분)


3. ** 리팩토링 (4시간)**
    - GameService -> LuckyGame 이름 수정 ✅
    - LuckyGame, GameManager 리팩토링 ✅
    
## [2-1]게임 로직 구현하기(8시간) - 수요일(7월 12일) 
1. ** 단위테스트 추가 학습 (4시간)**
    - 예시코드 및 자료 10개 이상 정리 ✅
...
2. ** 테스트 코드 작성 (2시간 30분)**
    - 정렬 관련 테스트 코드 작성(1시간) ✅
    - 분배 관련 기능 테스트 코드 작성(30분) ✅
    - 참가자 중에 같은 숫자 카드 3장을 가진 경우가 있는지 판단 테스트 코드 작성(30분) ✅
    - 아무거나를 선택해서 3개가 같은지 판단 테스트 코드 작성(30분) ✅
    
## [2-1]게임 로직 구현하기(8시간) - 목요일(7월 13일)

1. ** Controller와 View 분리 (4시간)**
    - GameView 구현 (2시간) ✅
    - GameBoardViewController 뷰 관련 로직 제거 (1시간) ✅
    - CardContainerView & BottomContainerView 리팩토링 (1시간) ✅

2. ** PR 반영 리팩토링 및 테스트 코드 마무리 (2시간)**
    - GameManager 테스트 코드 (1시간) ✅
    - LuckyCard 테스트 코드 (1시간) ✅
